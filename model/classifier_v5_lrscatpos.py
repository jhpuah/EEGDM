...
class MHAStack(nn.Module):
    def __init__(
        ...
        self.have_cross_norm = cross_norm
        if self.have_cross_norm:
            self.cross_norms = nn.ModuleList([nn.LayerNorm(d_kv_embed) for _ in range(self.cross_count)])
            for n in self.cross_norms:
                nn.init.constant_(n.bias, 0)
                nn.init.constant_(n.weight, 1.0)
        ...
        for idx, (s, n, l) in enumerate(zip(self.struct, self.norms, self.layers)):
            ...
            match s:
                ...
                case "c":
                    _y = y[cross_idx]
                    if self.have_cross_norm:
                        _y = self.cross_norms[cross_idx](_y)
...
class Classifier(nn.Module):
    def __init__(
        ...
        self.rev_rescale = rev_rescale
        if rev_rescale: assert not ("gate" in query and "std" in reduce)
        ...
        if embed_query:
            self.query_embed_linear = nn.ModuleList([
                nn.Linear(self.model.d_model, self.model.d_model) for _ in query
            ])
        if n_q > 0:
            match multi_query_merge:
                case "cat":
                    ...
                    if embed_query:
                        self.multi_query_unpack = partial(torch.chunk, chunks=n_q, dim=-1)
        ...
        if have_ch_pos_embed:
            self.ch_is_left = nn.Parameter(
                torch.tensor(list(map(lambda c: (int(c.split("-")[0][-1]) % 2) * 2 - 1, ch_order))).reshape(1, 1, 1, 1, 1, -1, 1),
                requires_grad=False
            )
        ...
        if n_tower > 1:
            self.t_pos_embed = nn.Parameter(torch.zeros(1, n_tower, 1, 1, d_embed))
            nn.init.trunc_normal_(self.t_pos_embed, std=INIT_STD, a=-INIT_STD, b=INIT_STD)
        else:
            self.t_pos_embed = 0
        if n_pool > 1:
            self.p_pos_embed = nn.Parameter(torch.zeros(1, 1, n_pool, 1, d_embed))
    ...
    def forward(self, input, data_is_cached=False):
        ...
        if self.embed_query:
            # print("1", all_tokens.shape)
            all_tokens = self.multi_query_unpack(all_tokens)
            # print("2", all_tokens[0].shape)
            embedded_tokens = []
    ...
    def compute_tokens(self, input):
        ...
        for at, r, rs in zip(all_tokens, self.reduce, self.rescale):
            at = rs(r(at, dim=-1))
            if self.rev_rescale: at = at / 2 + 0.5
            temp.append(at)
