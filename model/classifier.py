...
class Classifier(nn.Module):
    def __init__(
        ...
        for r in reduce:
            match r:
                case "mean": self.reduce.append(torch.mean)
                case "std": self.reduce.append(torch.std)
        ...
        match pool_merge:
            ...
            case "share":
                if pool_share_clst:
                    self.p_expand = n_pool
                else:
                    self.p_expand = -1
                    clst_shape[clst_P_dim] = n_pool
        ...
        self.cls_token = nn.Parameter(torch.rand(clst_shape))
        self.pos_embed = nn.Parameter(
            calc_diffusion_step_embedding(
                torch.arange(n_clst).unsqueeze(-1),
                d_embed
            ).reshape(1, 1, 1, n_clst, d_embed),
            requires_grad=False
        ) if final_stack_pos_embed else 0
        ...
        if final_stack_struct is not None:
            ...
            total_clst = np.prod(clst_shape[:-1]).item()
            self.final_stack_pos_embed = nn.Parameter(
                calc_diffusion_step_embedding(
                    torch.arange(total_clst).reshape(total_clst, 1),
                    d_embed
                ).reshape(1, total_clst, d_embed),
                requires_grad=False
            )
        ...
        match final_act:
            ...
            case "cls":
                assert (not cross_only and clst_shape[clst_P_dim] == clst_shape[clst_T_dim] == 1) or final_stack_struct is not None
    ...
    def forward(self, input, data_is_cached=False):
        ...
        x = self.cls_token.expand(B, -1, self.p_expand, -1, -1) + self.pos_embed
        # B T P N H
        effective_P = x.shape[2]
        ...
        for T, (Bn_PQCH, tower, ac_tower) in enumerate(zip(all_tokens.unbind(dim=2), self.tower, self.across_pool_tower)): # eliminate T
            for B__PQCH, layer, ac_layer in zip(Bn_PQCH.unbind(dim=1), tower, ac_tower): # eliminate n
                ...
                if ac_layer is not None:
                    ...
                    ac_x = ac_layer(ac_x)
                    x[T] = list(torch.chunk(ac_x, chunks=effective_P, dim=1)) # [B N H] * P

        x = [__x for _x in x for __x in _x] # flatten list
        x = torch.cat(x, dim=1)
    ...
    def compute_tokens(self, input):
        ...
        temp = []
        for at, r in zip(all_tokens, self.reduce):
            at = r(at, dim=-1)
            temp.append(at)
